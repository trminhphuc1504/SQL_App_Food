...
app.get("/api/like_res/restaurant/:res_id", async (req, res) => {
    try {
        ...
        const likes = await LikeRes.findAll({
            where: {
                res_id: resId, // Tìm các bản ghi có res_id bằng resId
            },
            include: [
                { model: User, attributes: ["full_name", "email"] }, // Bao gồm thông tin user
                { model: Restaurant, attributes: ["res_name", "image"] }, // Bao gồm thông tin nhà hàng
            ],
        });

        if (likes.length > 0) {
            res.status(200).json(likes); // Trả kết quả
        } else {
            res.status(404).json({
